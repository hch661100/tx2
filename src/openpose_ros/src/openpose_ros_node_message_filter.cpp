...
message_filters::Subscriber<sensor_msgs::Image>* image_sub_depth;
...
//        message_filters::Subscriber<sensor_msgs::Image> image_sub_depth;
	...
	c_y = cy_l = cy_r = 187.899 * scale_;
        ...
        image_sub_l = new message_filters::Subscriber<sensor_msgs::Image>(nh_, "image_left", 1);
        image_sub_depth = new message_filters::Subscriber<sensor_msgs::Image>(nh_, "image_depth", 1);
        sub_odom = new message_filters::Subscriber<nav_msgs::Odometry>(nh_,"odom", 1);
...
void calculate_3D_skeleton(image_recognition_msgs::PersonDetection pose_left_)
{
	...
	{	
		...
		pose_3d_list.push_back((left_hip_3d + right_hip_3d) / 2);
                ...
                pose_3d_list.push_back(Eigen::Vector3d(-1, -1, -1));
        }
...
void callback(const sensor_msgs::Image::ConstPtr& image_msg, const sensor_msgs::Image::ConstPtr& depth_msg, const nav_msgs::Odometry::ConstPtr& odom_msg);
...
int opRealTimeProcessing()
{
    ...
    TimeSynchronizer<sensor_msgs::Image, sensor_msgs::Image, nav_msgs::Odometry> sync(*image_sub_l, *image_sub_depth, *sub_odom, 10);
...
void callback(const sensor_msgs::Image::ConstPtr& image_msg, const sensor_msgs::Image::ConstPtr& depth_msg, const nav_msgs::Odometry::ConstPtr& odom_msg)
{
    ...
    //inputImage_l = matFromImage( image_msg ).clone();
    //imageDepth_ = matFromImage( depth_msg ).clone();
    //opn.convert_image_l(*image_msg);
    //opn.convert_image_depth(*depth_msg);
    //opn.odom_callback(odom_msg);
    /*
